...
public class GetFileList {
    ...
    public static void main(String[] args) throws IOException, GeneralSecurityException {
        ...
        // Define fields to retrieve from API
        String fields = "nextPageToken, files(id, name)";

        // Get a list of up to 10 files from the user's Google Drive account
        FileList result = service.files().list()
                .setPageSize(100)
                .setFields(fields)
                .execute();

        // Print out the file ID and name for each file in the list
    ...
    private static Credential getCredentials(final NetHttpTransport HTTP_TRANSPORT) throws IOException {
        // Load client secrets.
        InputStream in = GetFileList.class.getResourceAsStream(CREDENTIALS_FILE_PATH);
        if (in == null) {
